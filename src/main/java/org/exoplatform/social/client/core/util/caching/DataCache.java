 ...
 */
package org.exoplatform.social.client.core.util.caching;
...
public abstract class DataCache<K, V> extends DataMap<K, V>{
  ...
  /**
   * Create the CallableCreate class which inherits from {@link Callable} class.
   * It is provided to the FutureTask to invoke the {@link CallableCreate#call()} to take the result.
   * @author thanh_vucong
   *
   */
  class CallableCreate implements Callable<V> {
    ...
    public V call() {
      ...
      //This method to call create() method for ContainerImpl.factories(...)
      // create value.
      value = create(key);
      if (value == null) {
